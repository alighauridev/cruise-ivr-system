...
export async function GET(req: NextRequest) {
  ...
  if (leadId) {
    rows = await sql`
      SELECT * FROM ivr_configs WHERE user_id = ${userId} AND lead_id = ${leadId}
...
export async function POST(req: NextRequest) {
  ...
  const rows = await sql`
    ...
    VALUES (${ctx.effectiveUserId}, ${leadId ?? null}, ${name}, ${JSON.stringify(steps ?? [])})
    RETURNING *
  `;
...
export async function DELETE(req: NextRequest) {
  ...
  await sql`DELETE FROM ivr_configs WHERE id = ${id} AND user_id = ${ctx.effectiveUserId}`;
